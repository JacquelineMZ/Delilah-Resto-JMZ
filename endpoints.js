...
AutenticarTokenUsuario.use((req, res, next) => {
 ...
 const AutenticarTokenTodos = express.Router(); // Correcion de método, ahora si funciona... Tanto para usuario como para administrador, ya que depende unicamente de la variable firma, y esta es dada por la sesión del middleware
...
server.get('/producto/listauno/:ID_PRODUCTO',AutenticarTokenTodos,(req,res)=>{ //Condición 5: CRUD de productos (Administrador) // Condición 2: Listar productos: Mejora de permisos, ahora un usuario autenticado puede hacer uso de este endpoint
...
server.get('/producto/listartodos/',AutenticarTokenTodos,(req,res)=>{//Condición 5: CRUD de productos (Administrador) // Condición 2: Listar productos: Mejora de permisos, ahora un usuario autenticado puede hacer uso de este endpoint
...
server.post('/pedido/orden/',AutenticarTokenTodos,(req,res)=>{// Condición 3: Generar pedido: Mejora de permisos, un usuario autenticado puede crear un pedido
    const {
        ID_USUARIO,
        ID_ESTADOS,
        FECHA,
    ...
    // Condición 3: Generar pedido: En esta parte se realiza el guardado en la base de datos
...
//5. DELETE para eliminar un pedido
server.delete('/pedido/eliminar/:ID_ORDENES',AutenticarTokenAdministrador,(req,res)=>{//Condición 6: Control de acciones usuario no Administrador: Mejora, se realiza la eliminación de un pedido
    const ID_ORDENES = req.params.ID_ORDENES;
    sequelize.query('DELETE FROM ORDENES WHERE ID_ORDEN =:ID_ORDENES', 
    { replacements: { ID_ORDENES:ID_ORDENES }}
    ).then(resultados => res.json(resultados)
    ).catch(function (error) {
        res.status(401);
        res.json({error_presentado:error});
